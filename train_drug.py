...
def main():
    ...
    # drug name
	drugs = [1001,1004, 1005, 1006, 1007 ,1008, 1009, 1010 ,1011 ,1012 ,1013 ,1017, 1018, 1019, 1020, 1021,
	1022 ,1023 ,1024 ,1026 ,1028 ,1029 ,1030, 1031, 1032 ,1033 ,1036 ,1037, 1038 ,1043, 1046, 1047,
	1048 ,1049 ,1050 ,1052, 1053 ,1054 ,1057 ,1058, 1059 ,1060 ,1069, 1072, 1091, 1129, 1067, 1175,
	1184 ,1199 ,1239 ,1259 ,1268 ,1372 ,1373, 1375 ,1401 ,1428 ,1441 ,1449 ,1463, 1494]
	# drugs = [1012,1375,1008,1004,1013]
    ...
    ### 5-folder training for each drug 
	for drug in drugs:
		##### CCLE 
		x = pd.read_csv('./DATASETS/drug/expression.csv', index_col=0)
		y = pd.read_csv("./DATASETS/drug/dose_clean.csv", index_col=0)
		gene_A = pd.read_csv('./DATASETS/drug/exp_gene_A.csv', index_col=0)
		np.fill_diagonal(gene_A.values, 1)
		idmap = pd.read_csv("./DATASETS/drug/model_list_20200730.csv")
		y = y[y.DRUG_ID == drug]
		
		# using model_name to match y, using CCLE_ID to match x
		idmap = idmap.loc[:, ["model_id", "CCLE_ID"]]
		common_y = set(idmap["model_id"]).intersection(y.SANGER_MODEL_ID)
		idmap = idmap.loc[idmap["model_id"].isin(common_y), :]
		common_x = set(idmap["CCLE_ID"]).intersection(x.columns)
		idmap = idmap.loc[idmap["CCLE_ID"].isin(common_x), :]
		ic50 = y.set_index("SANGER_MODEL_ID").loc[idmap["model_id"], "LN_IC50"]
		x = x.loc[:, idmap["CCLE_ID"]].T
		quantile =  np.percentile(ic50.values, (25, 50, 75), interpolation='midpoint')
		
		x0 = x.values[ic50.values<quantile[0],]
		x1 = x.values[ic50.values>quantile[2],]
		x = pd.concat([pd.DataFrame(x0),pd.DataFrame(x1)])
		x = np.asarray(x)
		y = np.asarray([0] * x0.shape[0] + [1] * x1.shape[0])
		gene_A = gene_A.values
		...
		for train_portion in train_portions:
			...
			for re, (train_idx_val, idx_test) in enumerate(
				...
				print(
					' drug:{0},re: {1},train size: {2}, valid_size:{3}, test size: {4}'.format(
					   drug, re, idx_train.shape[0], idx_val.shape[0], idx_test.shape[0]
					)
				)
